<script lang="ts">
	...
	const form = superForm(formProp, {
		...
		onUpdated({ form }) {
			if (form.valid) {
				toast.success(
					formTarget === true
						? m['admin.genre.action.create_success']()
						: m['admin.genre.action.edit_success']()
				);
				formTarget = null;
			} else {
	...
	let deleteTarget = $state<Genre | null>(null);
	let deleting = $state(false);

	let formTarget = $state<Genre | true | null>(null);
	let formNameHu = $state('');
	let formNameEn = $state('');
	let formSaving = $state(false);

	const cols = columns(
		(genre) => (deleteTarget = genre),
		(genre) => {
			formTarget = genre;
			$formData.id = genre.id;
			$formData.name_hu = genre.name_hu;
			$formData.name_en = genre.name_en;
		}
	);
...
<div class="mx-auto w-full px-4 pt-16 md:w-4/5 md:px-0">
	...
	<div class="mb-6">
		...
		<div class="flex w-full flex-col gap-5 px-4 md:hidden">
			...
			{:else}
				{#each data as genre (genre.id)}
					<div>
						<h2 class="text-xl">{getLocale() === 'hu' ? genre.name_hu : genre.name_en}</h2>
						<p
							class="line-clamp-1 w-full overflow-hidden text-sm text-ellipsis text-muted-foreground"
						>
							{getLocale() === 'hu' ? m['admin.genre.name_en']() : m['admin.genre.name_hu']()}:
							{getLocale() === 'hu' ? genre.name_en : genre.name_hu}
						</p>
						<p
							class="line-clamp-1 w-full overflow-hidden text-sm text-ellipsis text-muted-foreground"
						>
						...
						<div class="mt-4 flex flex-row gap-2">
							<Button
								...
								onclick={() => {
									formTarget = genre;
									formNameHu = genre.name_hu;
									formNameEn = genre.name_en;
								}}
								variant="outline"
							>
								<Pencil size={16} />
...
>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title>
				{formTarget === true ? m['admin.genre.action.new']() : m['admin.genre.action.edit']()}
			</AlertDialog.Title>
		</AlertDialog.Header>
		<form method="POST" action={formTarget === true ? '?/create' : '?/update'} use:formEnhance>
			<div class="flex flex-col gap-4 py-2">
				<Form.Field {form} name="name_hu">
					<Form.Control>
						{#snippet children({ props })}
							<Form.Label>{m['admin.genre.name_hu']()}</Form.Label>
							<Input {...props} bind:value={$formData.name_hu} />
						{/snippet}
					</Form.Control>
					<Form.FieldErrors />
				</Form.Field>
				<Form.Field {form} name="name_en">
					<Form.Control>
						{#snippet children({ props })}
							<Form.Label>{m['admin.genre.name_en']()}</Form.Label>
							<Input {...props} bind:value={$formData.name_en} />
						{/snippet}
					</Form.Control>
					<Form.FieldErrors />
				</Form.Field>
...
>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title>{m['admin.genre.action.delete']()}</AlertDialog.Title>
			<AlertDialog.Description>
				<div class="flex flex-col gap-4">
					{#if data.find((x) => x.id === deleteTarget?.id)?.books.length}
						<span>
							{m['admin.genre.delete_dialog.has_books']()}
						</span>
					{/if}
					<span>
						{m['admin.genre.delete_dialog.description']({
							name:
								getLocale() === 'hu' ? (deleteTarget?.name_hu ?? '') : (deleteTarget?.name_en ?? '')
						})}
					</span>
				</div>
			</AlertDialog.Description>
				...
				use:enhance={({ formData }) => {
					...
					return async ({ result, update }) => {
						if (result.type === 'success') {
							toast.success(m['admin.genre.action.delete_success']());
							deleteTarget = null;
						} else {
							toast.error(m['messages.server_error']());
						}
